import pytest
import numpy as np
from maneuvers.classify import build_pipeline, train_with_grid_search, train_classifier
...
def _make_small_dataset(n=20, seed=0):
    ...
    y = []
    for i in range(n):
        seq = generate_synthetic_sequence(duration_s=5.0, fs=50, seed=int(rng.integers(0, 1_000_000)))
        feats = compute_features_from_sequence(seq)
        Xv, yv = feats.iloc[0:10][["accel_mag", "accel_smooth", "gyro_mag"]].mean().values, 'A' if i % 2 == 0 else 'B'
        X.append(Xv)
        y.append(yv)
    return np.vstack(X), y


@pytest.mark.skipif(not hasattr(__import__('importlib').import_module('importlib').util, 'find_spec') or __import__('importlib').util.find_spec('xgboost') is None, reason='xgboost not installed')
