...
def test_train_synthetic_creates_model(tmp_path):
    ...
    result = runner.invoke(app, ["train-synthetic", "--duration", "3", "--fs", "50", "--out", str(out)])
...
def test_detect_with_model(tmp_path):
    ...
    r2 = runner.invoke(app, ["detect-synthetic", "--duration", "3", "--fs", "50", "--threshold", "0.2", "--model", str(out)])
