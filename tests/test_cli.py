...
def test_train_synthetic_command(tmp_path):
    ...
    result = runner.invoke(app, ["train-synthetic", "--duration", "5", "--fs", "50", "--out", str(model_path)])
    assert result.exit_code == 0
...
def test_detect_with_model(tmp_path):
    ...
    # First train a model
    model_path = tmp_path / "test_model.joblib"
    train_result = runner.invoke(app, ["train-synthetic", "--duration", "5", "--fs", "50", "--out", str(model_path)])
    assert train_result.exit_code == 0

    # Now test detection with model
    result = runner.invoke(app, ["detect-synthetic", "--duration", "3", "--fs", "50", "--threshold", "0.2", "--model", str(model_path)])
    assert result.exit_code == 0
    assert "Detected" in result.output
...
def test_detect_with_invalid_model():
    ...
    result = runner.invoke(app, ["detect-synthetic", "--duration", "3", "--fs", "50", "--threshold", "0.2", "--model", "nonexistent.joblib"])
    assert result.exit_code == 0
...
def test_detect_with_output_file(tmp_path):
    ...
    result = runner.invoke(app, ["detect-synthetic", "--duration", "3", "--fs", "50", "--threshold", "0.2", "--out", str(output_path)])
    assert result.exit_code == 0
