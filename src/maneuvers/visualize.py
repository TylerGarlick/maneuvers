...
def plot_3d_segments(
    seq, segments: List[Tuple[int, int]], labels: List[str] = None, out_path: str = "flight_path_3d.png", interactive: bool = False
):
    ...
    t_norm = (seq.timestamps - seq.timestamps.min()) / (seq.timestamps.max() - seq.timestamps.min())
    ax.scatter(seq.accel[:, 0], seq.accel[:, 1], seq.accel[:, 2], c=t_norm, cmap='viridis', alpha=0.6, s=1)
    ...
    for i, (s, e) in enumerate(segments):
        ...
        ax.scatter(seq.accel[s:e, 0], seq.accel[s:e, 1], seq.accel[s:e, 2], 
                   color=colors[i % len(colors)], s=10, label=label)
        # Mark start and end
        ax.scatter(seq.accel[s, 0], seq.accel[s, 1], seq.accel[s, 2], 
                   color='red', marker='^', s=50)  # start
        ax.scatter(seq.accel[e-1, 0], seq.accel[e-1, 1], seq.accel[e-1, 2], 
                   color='green', marker='v', s=50)  # end
